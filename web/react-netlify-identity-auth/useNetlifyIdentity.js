...
const useNetlifyIdentity = ({ url: _url }) => {
  ...
  // A flag for signifying whether the identity module is ready
  ...
  const authorizedFetch = useCallback(async (uri, options) => {
  ...
  useEffect(() => {
    if (goTrueToken) {
      ...
      const timeToRefresh = Math.max((new Date(goTrueToken.expires_at)).getTime()
        - (new Date()).getTime() - FOUR_MINUTES, 0);
  ...
  const pendingEmailUpdate = useMemo(
    () => ((user?.email !== user?.new_email) && user?.new_email), [user],
  );
